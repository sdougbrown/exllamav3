...
//
// Design constraints established by the P5 Stage 1/2 probes (2026-09-08 report):
//  - Kernel waits only. hipStreamWaitValue32 requires a hipMallocSignalMemory pointer
//    (hip_runtime_api.h:3080-3113) and that allocation fails on gfx1201, so the CUDA build's
//    cuStreamWaitValue32 memops path is NOT ported. exl3_moe_cpu_set_memops is an accepted
//    no-op so the Python side stays uniform.
//  - Release/acquire flag pairing at system scope: __hip_atomic_load/store with
//    __HIP_MEMORY_SCOPE_SYSTEM (compile-validated on gfx1201; probe A2-A6).
//  - __nanosleep is undeclared on ROCm 7.2.4 and __builtin_amdgcn_s_sleep rejects runtime
//    arguments: backoff uses literal immediate buckets only.
//  - s_sleep is cycle-based, so the ~30 s wait timeout maps to a poll budget calibrated at
//    startup by measuring the kernel's no-publisher cadence once (probe B0). On timeout the
//    wait kernel release-stores abort=1 and returns, so a stream can never hang on a
//    dead/hung worker; the host-side watchdog remains the recovery path.
...
static unsigned long long moe_wait_poll_budget(hipStream_t stream)
{
    ...
    if (g_polls_ready.exchange(true, std::memory_order_acq_rel))
    {
        ...
        // ~3 us per poll epoch at gfx1201 sclk (probe-measured ~9.5 us worst case; the
