...
Two retained gfx12 optimizations from `d189d3a` and `0bbf261` carry that target further. The fixed-K QSA selector is native only for gfx1200/gfx1201 wave32 inputs with fp16 scores, int32 indices, `k == 512`, a 2D score matrix with 128-aligned row stride, and no `t_ptr` / `t_seq` override; every other shape or device uses the Python fallback. It preserves the exact fp16 bit-key order used in tests: `-inf` and negative NaNs stay out, positive NaNs rank above `+inf`, and threshold ties break by ascending index. On the 128K-shape microbenchmark, the native path measured 6.646 ms versus 36.214 ms for the fallback, a 5.46x speedup.
